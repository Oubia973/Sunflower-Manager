...
const FACTION_BADGE = {
  sunflorians: imgfactions,
  nightshades: imgfactions,
  bumpkins: imgfactions,
  goblins: imgfactions,
};
// const FACTION_PET = {
...
function FactionCard({ factionKey, factionData, dataSet, memberFaction, isEligible, factionsItems, imgExchng, imgprodit, imgbuyit }) {
  ...
  const prodIcon = getIconMeta(imgprodit, imgprodit?.props?.src, "Self production");
  const buyIcon = getIconMeta(imgbuyit, imgbuyit?.props?.src, "Marketplace");
  const exchangeIcon = getIconMeta(imgExchng, imgExchng?.props?.src, "Marketplace");
        ...
        <table className="factions-table">
          <thead>
            <tr>
              <th>Item</th>
              {/* <th>XP</th> */}
              <th className="factions-col-cost">
                <span className="factions-head-inline">
                  <span>Cost</span>
                  <img src={prodIcon.src} alt="" title={prodIcon.title} className="factions-head-icon factions-head-icon-prod" />
                </span>
              </th>
              <th className="factions-col-cost factions-col-cost-buy">
                <span className="factions-head-inline">
                  <span>Bought</span>
                  <img src={buyIcon.src} alt="" title={buyIcon.title} className="factions-head-icon factions-head-icon-buy" />
                </span>
              </th>
            </tr>
          </thead>
...
function getIconMeta(icon, fallbackSrc, fallbackTitle) {
  if (typeof icon === "string") {
    return { src: icon, title: fallbackTitle };
  }

  const src = icon?.props?.src || fallbackSrc;
  const title = icon?.props?.title || fallbackTitle;
  return { src, title };
}
