...
export default function PetsTable() {
  ...
  const CATEGORY_IMG = {
    ...
    Horse: imgpetHorse,
    Bull: imgpetBull,
    Hamster: imgpetHamster,
    Penguin: imgpetPenguin,
    Ram: imgpetRam,
    Dragon: imgpetDragon,
    Phoenix: imgpetPhoenix,
    Griffin: imgpetGriffin,
    Warthog: imgpetWarthog,
    Wolf: imgpetWolf,
    Bear: imgpetBear,
  };
  ...
  if (petView === "pets") {
    ...
    const rows = categories.map(({ cat, petName: rowPetName }) => {
      ...
      const catImgPath = CATEGORY_IMG[cat] || imgna;
      let curNrg = 0;
      let petLvl = 0;
      let energySfl = 0;
      let totalNrg = 0;
      ...
      const itemIcons = fetchItems.map(comp => {
        const cimg = petit?.[comp]?.img || imgna;
      ...
      <table className="table">
        <thead>
          <tr>
            ...
            {isColVisible(petCols, 6) ? <th className="thcenter">Current <img src={imglightning} alt="" className="itico" title="Energy" /></th> : null}
            {/* <th className="thcenter">Exp</th> */}
            {isColVisible(petCols, 7) ? <th className="thcenter">Requests</th> : null}
            {isColVisible(petCols, 8) ? <th className="thcenter"><img src={imglightning} alt="" className="itico" title="Energy" /></th> : null}
            {isColVisible(petCols, 9) ? <th className="thcenter">Cost {imgprodit}</th> : null}
            {isColVisible(petCols, 10) ? <th className="thcenter">Bought {imgbuyit}</th> : null}
            {isColVisible(petCols, 11) ? <th className="thcenter"><img src={imglightning} alt="" className="itico" title="Energy" />/{imgprodit}</th> : null}
            {isColVisible(petCols, 12) ? <th className="thcenter"><img src={imglightning} alt="" className="itico" title="Energy" />/{imgbuyit}</th> : null}
          </tr>
        </thead>
        <tbody>{rows}</tbody>
      </table>
  ...
  if (petView === "components") {
    ...
    const rows = compNames.map((c, index) => {
      ...
      const catIcons = catArr.map(cat => {
        ...
        const img = CATEGORY_IMG[cat] || imgna;
        return (
      ...
      <table className="table">
        <thead>
          <tr>
            ...
            {isColVisible(compCols, 2) ? <th className="thcenter"><img src={imglightning} alt="" className="itico" title="Energy" /></th> : null}
            {isColVisible(compCols, 3) ? <th className="thcenter">Yield</th> : null}
            {isColVisible(compCols, 4) ? <th className="thcenter">Cost {imgprodit}</th> : null}
            {isColVisible(compCols, 5) ? <th className="thcenter">Bought {imgbuyit}</th> : null}
