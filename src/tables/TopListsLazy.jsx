...
export default function TopListsLazyTable() {
  ...
  const rows = useMemo(() => {
    ...
    activeTopNames.forEach((topName) => {
      ...
      list.forEach((entry, idx) => {
        ...
        if (!rowById.has(key)) {
          const base = { rank: "", id: entryId, lvl: "", lvlDisplay: "" };
        ...
        if (Number.isFinite(entryLvl) && entryLvl > Number(row.lvl || 0)) {
          row.lvl = entryLvl;
          row.lvlDisplay = entry.levelLabel || entryLvl;
        }
  ...
  const stickyWidths = useMemo(() => {
    ...
    const lvlLen = rows.reduce((max, row) => Math.max(max, String(row?.lvlDisplay ?? row?.lvl ?? "").length), String("LVL").length);
    ...
    >
      <div className="toplists-toolbar-row">
        <div ref={toolbarRef} className="toplists-toolbar">
          ...
          >
            <img src={imgsearch} alt="" className="itico" />
          </button>
          <button
            type="button"
          ...
          {(loadingCategories || anyLoadingCategory || loadingToplists) ? (
            <img src={imgsyncing} alt="" className="itico" title="Loading" />
          ) : null}
        </div>
      </div>
      {!categories.length ? (
            ...
            {sortedRows.map((row, i) => (
              <tr key={`${row.rank || i}-${row.id || i}`}>
                {columns.map((column) => (
                  ...
                  >
                    {formatCell(column.key === "lvl" ? (row?.lvlDisplay || row?.lvl) : row?.[column.key])}
                  </td>
                ))}
              </tr>
            ))}
