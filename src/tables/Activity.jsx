...
function getWeekResetKey(value, resetDay = 1) {
    const date = parseActivityDate(value);
    if (Number.isNaN(date.getTime())) return "";
    date.setHours(0, 0, 0, 0);
    const normalizedResetDay = Number.isFinite(Number(resetDay)) ? Number(resetDay) : 1;
    const currentDay = date.getDay();
    const diff = (currentDay - normalizedResetDay + 7) % 7;
    date.setDate(date.getDate() - diff);
    return date.toISOString().slice(0, 10);
}

function toDateValue(activityItem) {
    const dt = parseActivityDate(activityItem?.date);
...
function parseTradeDate(value) {
    ...
    if (!raw) return null;
    const compactMatch = raw.match(/^(\d{2})(\d{2})(\d{2})$/);
    if (compactMatch) {
        const [, mm, dd, yy] = compactMatch;
        const parsed = new Date(2000 + Number(yy), Number(mm) - 1, Number(dd), 12, 0, 0);
...
function TradeSummaryTable({ items, metric, selectedItems, setSelectedItems }) {
    ...
    const allSelected = sortedItems.length > 0 && sortedItems.every((item) => selectedItems.includes(item.name));

    return (
        <table className="activity-trades-table">
            <thead>
                <tr>
                    <th className="activity-trades-checkbox-header">
                        <input
                            type="checkbox"
                            checked={allSelected}
                            onChange={() => setSelectedItems(allSelected ? [] : sortedItems.map((item) => item.name))}
                            aria-label={allSelected ? "Deselect all items" : "Select all items"}
                        />
                    </th>
                    <th>Item</th>
                    <th className="activity-trades-metric-header">Qty</th>
                    <th className="activity-trades-metric-header"><img src={imgsfl} alt="Flower" className="itico" /></th>
            ...
            <tbody>
                {sortedItems.map((item) => (
                    <tr key={item.name}>
                        <td className="activity-trades-checkbox-cell">
                            <input
                                type="checkbox"
                                checked={selectedItems.includes(item.name)}
                                onChange={() => setSelectedItems((prevSelectedItems) => (
...
export default function ActivityTable() {
    ...
    useEffect(() => {
        const availableNames = filteredTradeSummaryItems.map((item) => item?.name).filter(Boolean);
        if (availableNames.length === 0) {
            setSelectedTradeItems([]);
            return;
        }
        setSelectedTradeItems((prevSelectedTradeItems) => {
            const nextSelectedItems = prevSelectedTradeItems.filter((itemName) => availableNames.includes(itemName));
            return nextSelectedItems.length > 0 ? nextSelectedItems : availableNames;
        });
    }, [filteredTradeSummaryItems]);
    useEffect(() => {
    ...
    if (activityDisplay === "day") {
        return setActivityDay(activityData, dataSetFarm, ui, ctx);
    }

    if (activityDisplay === "item") {
        return setActivityItem(activityData, dataSetFarm, ui, ctx);
    }

    if (activityDisplay === "quest") {
        return setActivityQuest(activityData, dataSetFarm, ui, ctx);
    }

    if (activityDisplay === "trades") {
        return setActivityTrades(
            activityData,
            dataSetFarm,
            ui,
            ctx,
            filteredTradeSummaryItems,
            filteredTradeDailySeries,
            selectedTradeItems,
            setSelectedTradeItems,
        );
    }
...
function setActivityDay(activityData, dataSetFarm, ui, ctx) {
    ...
    if (activityData[0]) {
        ...
        const ximgdchest = <i><img src={imgsynced} alt='' className="itico" title="Tickets from daily chest" style={{ width: `20px`, height: `20px` }} /></i>;
        ...
        const imgdeliv = <i><img src={imgdelivery} alt='' className="resico" title="Deliveries" style={{ width: `20px`, height: `20px` }} /></i>;
        const imgchore = <i><img src={imgchores} alt='' className="resico" title="Chores" style={{ width: `20px`, height: `20px` }} /></i>;
        const imgBountyChicken = <i><img src={imgchkn} alt='' className="itico" title="Bounty Chickens" /></i>;
        const imgBountyBarn = <i><img src={imgcow} alt='' className="itico" title="Bounty Barn" /></i>;
        const imgBountyPoppy = <i><img src={imgpoppy} alt='' className="itico" title="Bounty Poppy" /></i>;
        ...
        const tableContent = dayRows.map(({ activityItem, prevDayActivity }) => {
            const endDate = parseActivityDate(activityItem.date);
        ...
        const tableHeader = (
            <thead>
                ...
                <tr>
                    {xListeColActivity[9][1] === 1 ? <th className="thcenter" style={deliveryLeftStyle}>Cost {imgprodit}</th> : null}
                    {xListeColActivity[10][1] === 1 ? <th className="thcenter">Bought {imgbuyit}</th> : null}
                    ...
                    {xListeColActivity[14][1] === 1 ? <th className="thcenter" style={poppyLeftStyle}>Cost {imgprodit}</th> : null}
                    {xListeColActivity[15][1] === 1 ? <th className="thcenter">Bought {imgbuyit}</th> : null}
...
function setActivityItem(activityData, dataSetFarm, ui, ctx) {
    const {
        data: { dataSet },
        actions: {
            handleUIChange,
            ...
            if (compoHarvested[element] > 0 || compoBurn[element] > 0 || compoTraded[element] > 0) {
                ...
                {xListeColActivityItem[7][1] === 1 ? <td className="tdcenter">{icostp2po && frmtNb(icostp2po)}</td> : null} */}
                        ...
                        {xListeColActivityItem[9][1] === 1 ? Object.entries(foodBuild).map(([itemName]) => (
                            (food[itemName]) ? (<td className="tdcenterbrd" key={`${element}-${itemName}`}>{formatActivityItemCell(foodBuild[itemName][element])}</td>) : null)) : null}
                    </tr>
                );
            }
        });
            ...
            <thead>
                <tr>
                    ...
                    {xListeColActivityItem[3][1] === 1 ? <th className="thcenter">
                        ...
                        Burn</th> : null}
                    {xListeColActivityItem[4][1] === 1 ? <th className="thcenter">Cost {imgprodit}</th> : null}
              ...
              {xListeColActivityItem[7][1] === 1 ? <th className="thcenter">OpenSea</th> : null} */}
                    {xListeColActivityItem[8][1] === 1 ? <th className="tdcenterbrd"><i><img src={imgexchng} title="Traded" className="itico" /></i></th> : null}
                    {xListeColActivityItem[8][1] === 1 ? <th className="tdcenterbrd"><i><img src={imgsfl} title="SFL" className="itico" /></i></th> : null}
                    {xListeColActivityItem[9][1] === 1 ? <th className="tdcenterbrd"><i><img src={imgdelivBoard} title="Deliveries burn" className="itico" /></i></th> : null}
                    {xListeColActivityItem[9][1] === 1 ? Object.entries(foodBuild).map(([itemName]) => (
...
function setActivityTrades(activityData, dataSetFarm, ui, ctx, tradeSummaryItems, tradeDailySeries, selectedTradeItems, setSelectedTradeItems) {
    const {
        actions: {
            handleUIChange,
        }
    ...
    const visibleTradeDailySeries = Array.isArray(tradeDailySeries)
        ? tradeDailySeries.map((day) => {
            const nextItems = {};
            Object.entries(day?.items || {}).forEach(([name, item]) => {
                if (selectedItemSet.has(name)) {
                    nextItems[name] = item;
                }
            });
            return {
        ...
        : [];
    const totalPrice = visibleSummaryItems.reduce((sum, item) => sum + Number(item?.price || 0), 0);
        ...
        <div style={{ width: "100%", height: "100%", display: "flex", flexDirection: "column", minHeight: 0 }}>
            <div className="modalgraph-buttons" style={{ marginBottom: 12 }}>
                <div className="modalgraph-header-left" style={{ width: "100%", display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
                    ...
                    >
                        <img src={imgsfl} alt="" style={{ width: 14, height: 14, objectFit: "contain" }} />
...
function setActivityQuest(activityData, dataSetFarm, ui, ctx) {
    const {
        actions: {
            handleUIChange,
        }
    ...
    if (activityData[0]) {
        ...
        const tableContent = uniqueQuestsArray.reverse().map((uniqueQuest) => {
            const rowKey = getQuestKey(uniqueQuest);
            ...
            xfrom = uniqueQuest.fromIcon || getNpcIconPath(ofrom);
            const ximgfrom = <img src={xfrom} alt="" title={ofrom} onError={(e) => { e.currentTarget.onerror = null; e.currentTarget.src = imgna; }} style={{ width: '20px', height: '20px' }} />;
            ...
            return (
                <tr key={rowKey}>
                    ...
                    {xListeColActivityQuest[3][1] === 1 ? columns.map((value, index) => (
                        (value === "X") ? (<td className="tdcenterbrd" key={`${rowKey}-${index}`} style={{ backgroundColor: 'rgba(0, 110, 0, 0.39)' }} title='completed'></td>) :
                            (value === "-") ? (<td className="tdcenterbrd" key={`${rowKey}-${index}`} style={{ backgroundColor: 'rgba(110, 0, 0, 0.39)' }} title='skipped'></td>) :
                                (value === ".") ? (<td className="tdcenterbrd" key={`${rowKey}-${index}`} style={{ backgroundColor: 'rgba(110, 110, 0, 0.39)' }} title='not done'></td>) :
                                    (<td className="tdcenterbrd" key={`${rowKey}-${index}`}></td>))) : null}
                </tr>
            );
        });
        const tableHeader = (
            <thead>
                <tr>
                    ...
                    {xListeColActivityQuest[3][1] === 1 ? Object.entries(dayKeys).map((date, index) => (
                        (<th className="tdcenterbrd" key={date[0]} style={{ fontSize: '8px' }}>{formatDateAndSupYr(activityData[index].date)}</th>))) : null}
                </tr>
            </thead>
        );
        const table = (
...
function setActivityTot(activityData, xContext, dataSetFarm, dataSet, prevDayActivity = null) {
    ...
    dataEntries.map((value, index) => {
        ...
        const endDate = parseActivityDate(DataContext.date);
        const dateSeason = parseActivityDate(dateSeasonConst);
        const dateSeasonDailyStart = parseActivityDate(dateSeasonDailyStartConst);
        const currentWeekKey = getWeekResetKey(DataContext.date, 1);
        const previousWeekKey = prevDayActivity ? getWeekResetKey(prevDayActivity.date, 1) : "";
        const weeklyAnchorDay = !prevDayActivity || currentWeekKey !== previousWeekKey;
        ...
        const isSeasonDay = endDate.setHours(0, 0, 0, 0) >= dateSeason.setHours(0, 0, 0, 0);
        const isDeliverySeasonDay = parseActivityDate(DataContext.date).setHours(0, 0, 0, 0) >= dateSeasonDailyStart.setHours(0, 0, 0, 0);
        //const curw = ((endDate.getDate()) / 8);
        //const isweeklyactday = Number.isInteger(curw) || (endDate.getDate() === dateSeason.getDate() && endDate.getMonth() === dateSeason.getMonth());
        //const wactdone = isweeklyactday && wklactivity[Math.floor(curw) + 1];
        if (((xContext === "day" && i === 0) || xContext !== "day") && isSeasonDay) {
            ...
            totDelivEntries.map(([item]) => {
                ...
                const deliveryCompletedToday = OrderItem?.completed && deliveryCompletedAt && !Number.isNaN(deliveryCompletedAt.getTime())
                    ...
                    : false;
                const istkt = getImageFileName(rewardImg) === getImageFileName(dataSet.imgtkt)
            ...
            const deliveryMaxReward = !isShelly && !isPreSeason && isDeliverySeasonDay && istkt && (rewardQty || 0);
            ...
            totChoreEntries.map(([item]) => {
                ...
                if (isTicketChore && weeklyAnchorDay) {
            ...
            Object.values(DataContext.data.bounties || {}).forEach((bountyItem) => {
                ...
                if (isTicketReward && weeklyAnchorDay) {
...
function setActivityTotQuest(activityData, dataSetFarm) {
    ...
    dataEntries.map((value, index) => {
        ...
        totChoreEntries.map(([item]) => {
            ...
            Quest[i] = {
                from: choreFrom,
                fromIcon: String(choreItem?.itemimg || imgchores),
                questCategory: "Chore",
        ...
        totBountyEntries.map(([item, bountyItem]) => {
            ...
            const bountyItemImg = String(bountyItem?.itemimg || imgpirateBounty);
            ...
            Quest[i] = {
                from: "bounty",
                fromIcon: bountyRewardImg || imgpirateBounty,
...
function formatDate(xDate, setUTC) {
    const currentDate = (xDate instanceof Date) ? xDate : parseActivityDate(xDate);
    var day = "";
    var month = "";
    var year = "";
    if (setUTC) {
