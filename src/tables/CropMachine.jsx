...
export default function CropMachineTable() {
    ...
    if (cmSource?.itables?.it && cmSource?.CropMachine && dataSet?.options) {
        ...
        const tableContent = Keys.map((element) => {
            ...
            const dailyRestockSflRaw = (Number(dataSet?.options?.gemsRatio || 0) > 0)
                ? (dailyRestockGems * Number(dataSet.options.gemsRatio))
                : 0;
            const restockCountedInDaily = !!dataSet.options?.restockCostDaily;
            const dailyRestockSfl = restockCountedInDaily ? dailyRestockSflRaw : 0;
            ...
            const dailyTooltip = {
                ...
                dailyRestockSfl: dailyRestockSflRaw,
            ...
            return (
                <React.Fragment key={element}>
                    {(element === actualLastCrop) ? <tr key={`${element}-unavailable`}><td colSpan={unavailableColSpan} style={{ textAlign: "center", fontWeight: "bold" }}>
        ...
        const tableHeader = (
            <thead>
                <tr>
                    ...
                    {showCol(8) ? <th className="thcenter"><div style={{ fontSize: "11px" }}>Total</div>Cost {imgprodit}</th> : null}
