...
async function fetchMarket(dataSetFarm, API_URL) {
  const farmId = dataSetFarm?.frmid;
  const userName = dataSetFarm?.username;
  if (!farmId) return null;

  const cacheKey = `${API_URL}|${farmId}`;
  const cached = marketRequestCache.get(cacheKey);
  const now = Date.now();

  if (cached?.data && now - Number(cached.fetchedAt || 0) < MARKET_CACHE_TTL_MS) {
    return cached.data;
  }
  ...
  try {
    ...
    marketRequestCache.delete(cacheKey);
    throw error;
  }
}
...
export default function MarketTable() {
  const {
    ...
    ui: { selectedInv, xListeColBounty },
    config: { API_URL },
  } = useAppCtx();
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      setLoading(true);
      try {
        const data = await fetchMarket(dataSetFarm, API_URL);
        if (!ignore) setMarket(data);
      } catch (e) {
        console.log(e);
        if (!ignore) setMarket(null);
      } finally {
  ...
  }, [selectedInv, dataSetFarm?.frmid, dataSetFarm?.username, API_URL]);
...
function OffersTable({ elements, dataSet, dataSetFarm, xListeColBounty }) {
  ...
  const rows = keys.map((k) => {
    ...
    let itemObj = { img: imgna, cost: 0, costp2pt: 0, pricemsfl: 0 };
    ...
    const ico = <img src={itemObj?.img || imgna} alt="" className="nodico" title={itemName} />;
      ...
      <table className="table">
        <thead>
          <tr>
            ...
            {xListeColBounty?.[4]?.[1] === 1 ? <th className="thcenter">Cost {imgSFL}</th> : null}
...
function TradesTable({ elements, dataSet, dataSetFarm, xListeColBounty }) {
  ...
  const rows = keys.map((k) => {
    ...
    let itemObj = { img: imgna, cost: 0, costp2pt: 0 };
      ...
      <table className="table">
        <thead>
          <tr>
            ...
            {xListeColBounty?.[4]?.[1] === 1 ? <th className="thcenter">{imgExchng}</th> : null}
            {xListeColBounty?.[4]?.[1] === 1 ? <th className="thcenter">Prod {imgprodit}</th> : null}
