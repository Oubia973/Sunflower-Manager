...
function ModalDlvr({
  ...
  const ximgno = <img src={imgcancel} alt="" />;
  const ximgalready = <img src={imgalready} alt="" />;
  const ximgyes = <img src={imgconfirm} alt="" />;
  const ximgrdy = <img src={imgrdy} alt="" />;
  const ximgheart = <img src={imglove} alt="" style={{ width: '12px', height: '12px' }} />;
  const imgtktname = getImageFileName(imgtkt);
  ...
  const getOrderRewardData = (orderItem) => {
    ...
    const istkt = rewardFileName === imgtktname || String(rewardImg).includes("ticket");
    ...
    const isgems = rewardImg === imggem || String(rewardImg).includes("gem");
  ...
  function setDeliveries() {
    ...
    inventoryEntries.forEach(([item], index) => {
      ...
      if (!deliveryCategoryTotals[rewardCategory]) {
        deliveryCategoryTotals[rewardCategory] = {
          ...
          rewardImg: rewardCategory === "coins" ? imgcoins : rewardCategory === "sfl" ? imgsfl : rewardCategory === "ticket" ? imgtkt : rewardCategory === "gems" ? imggem : (rewardImg || imgna),
          rewardDone: 0,
          rewardTotal: 0,
          costDone: 0,
          costTotal: 0,
    ...
    const totCompImg = <img src={imgcropBucket} alt="" className="resico" />;
    ...
    const renderBountyCard = (bountyName, bountyItem, bountyKey) => {
      ...
      const bountyRewardFileName = getImageFileName(bountyItem.rewardimg);
      bntyTktTot += (bountyRewardFileName === imgtktname) ? Number(bountyItem[key("reward")] || 0) : 0;
      bntyTktTotDone += bountyItem.completed && (bountyRewardFileName === imgtktname) ? Number(bountyItem[key("reward")] || 0) : 0;
      const rewardStyle = {
