import { imgcopy } from "./constants/images.js";
...
export function promptInfo(message, title = "Please wait", buttonLabel = "Got it", options = {}) {
  const closeOnBackdrop = options?.closeOnBackdrop !== false;
  return new Promise((resolve) => {
    const overlay = document.createElement("div");
    overlay.style.position = "fixed";
  ...
  return new Promise((resolve) => {
    ...
    const normalizedChoices = safeChoices.length > 0
      ? safeChoices.map((choice, index) => ({
        ...
        labelIconSrc: choice?.labelIconSrc ? String(choice.labelIconSrc) : "",
      }))
      : [
    ...
    normalizedChoices.forEach((choice, index) => {
      ...
      if (choice.labelIconSrc) {
        button.style.display = "inline-flex";
        button.style.alignItems = "center";
        button.style.gap = "6px";
        const labelIcon = document.createElement("img");
        labelIcon.src = choice.labelIconSrc;
        labelIcon.alt = "";
        labelIcon.style.width = "14px";
        labelIcon.style.height = "14px";
        labelIcon.style.objectFit = "contain";
        button.appendChild(labelIcon);
      }
