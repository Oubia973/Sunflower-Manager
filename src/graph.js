...
function withAlpha(color, alpha, fallback = "rgba(95, 219, 255, 1)") {
  ...
  const normalizedAlpha = Number.isFinite(safeAlpha) ? Math.min(1, Math.max(0, safeAlpha)) : 1;
  const rawColor = String(color || "").trim();
  if (!rawColor) {
    return fallback.replace(/1(\.0+)?\)?$/, `${normalizedAlpha})`);
  }

  if (rawColor.startsWith("#")) {
    const hex = rawColor.slice(1);
  ...
  const hslMatch = rawColor.match(/^hsla?\(([^)]+)\)$/i);
  if (hslMatch) {
  ...
  if (typeof document !== "undefined") {
    ...
    if (ctx) {
      ...
      if (computed.startsWith("#")) {
        return withAlpha(computed, normalizedAlpha, fallback);
      }
      const computedMatch = computed.match(/^rgba?\(([^)]+)\)$/i);
      if (computedMatch) {
        const parts = computedMatch[1].split(",").map((part) => part.trim());
        if (parts.length >= 3) {
          return `rgba(${parts[0]}, ${parts[1]}, ${parts[2]}, ${normalizedAlpha})`;
        }
      }
...
const categoryGroups = {
  "crops": ["crop"],
  "wood minerals": ["wood", "mineral", "gem", "salt"],
...
function Graph({ data, quantityData = [], vals, dataSetFarm, graphMeta = {}, selectedCategory = 'all', legendResetToken = 0, isLoading = false, quantityItemId = "", showQuantity = true, onSoloItem }) {
  ...
  const idName = useMemo(() => {
    ...
    Object.keys(graphMeta || {}).forEach((idKey) => {
      ...
      map[id].img = map[id].img || meta.img || imgna;
  ...
  const quantityDataset = useMemo(() => {
    ...
    if (!Number.isFinite(selectedId) || !Array.isArray(sourceData) || sourceData.length === 0) return null;

    const meta = graphMeta?.[selectedId] || graphMeta?.[String(selectedId)] || idName[selectedId] || {};
    const label = String(meta?.name || `#${selectedId}`).trim();
    const itemColor = String(meta?.color || "#6b7280").trim();
    const quantityFill = withAlpha(itemColor, 0.18, "rgba(95, 219, 255, 0.18)");
    const quantityBorder = withAlpha(itemColor, 0.38, "rgba(95, 219, 255, 0.38)");
    const rows = [...sourceData]
      .map((entry) => ({ entry, parsedDate: parseGraphDate(entry?.date) }))
      .filter(({ entry, parsedDate }) => parsedDate && Number(entry?.id) === selectedId)
      .sort((a, b) => a.parsedDate - b.parsedDate);

    if (rows.length < 1) return null;

    const quantityRows = rows.map(({ entry, parsedDate }) => {
      const rawQuantity = entry?.quantityDelta;
      const quantityValue = rawQuantity === null || rawQuantity === undefined || rawQuantity === "" ? null : Number(rawQuantity);
      return {
        parsedDate,
    ...
    const validRows = quantityRows.filter((row) => Number.isFinite(row.quantity) && row.quantity > 0);
    if (validRows.length < 1) return null;

    const quantityPoints = validRows.map((row) => ({
      x: row.parsedDate,
      y: row.quantity,
      unit: row.unit,
  ...
  const chartDatasets = useMemo(() => {
    ...
    return [quantityDataset, ...datasets];
  ...
  const boostNftLegendOptions = useMemo(() => {
    return datasets
      ...
      .map((dataset) => ({
        value: dataset.key,
        label: dataset.label,
        iconSrc: normalizeServerImageUrl(dataset.icon || imgna),
      }));
  }, [datasets, boostKeys]);
  ...
  const tooltipLabel = (context) => {
    ...
    if (context.dataset?.kind === "quantity" || context.dataset?.type === "bar") {
      return `${datasetLabel}: ${frmtNb(value)} sold`;
    }
    if (vals === 'price') {
  ...
  }, [chartDatasets, isLogarithmicScale, vals, effectiveHiddenLabels, selectedCategory, currentSeason]);
          ...
          datasets.map((dataset) => {
            ...
            return (
              ...
              >
                ...
                >
                  <img src={normalizeServerImageUrl(dataset.icon || imgna)} alt="" className="graph-legend-item-icon" />
