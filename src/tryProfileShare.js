...
export function buildTryProfilePayload(
  ...
  selectedTables.forEach((tableName) => {
    ...
    const rows = Object.entries(table)
      .filter(([, value]) => (
        tableName === "skill"
          ? Number(value?.leveltry ?? value?.level ?? 0) > 0
          : Number(value?.tryit || 0) === 1
      ))
      .map(([name, value]) => [
        name,
        String(value?.boost || ""),
    ...
    Object.entries(table).forEach(([name, value]) => {
      ...
      const skillPointCostAtLevel = (level) => (
        level > 0 ? getSkillPointsAtLevel(value, level) : 0
      );
      summaryBoostChanges.push([
        String(tableName || ""),
        String(name || ""),
        tryVal > activeVal ? "added" : "removed",
        String(value?.img || ""),
        sharedCat,
        String(value?.boost || ""),
        tableName === "skill"
          ? Math.abs(skillPointCostAtLevel(tryVal) - skillPointCostAtLevel(activeVal))
