...
export default function DList({
    ...
    function computePos() {
        ...
        const availableListHeight = (openDown ? spaceBelow : spaceAbove) - popupChromeHeight;
        const maxListH = Math.max(0, Math.min(maxListCap, availableListHeight));
        ...
        const clampedRight = Math.max(PAD, Math.round(window.innerWidth - r.right));
        const base = {
            ...(alignRight ? { right: clampedRight } : { left: clampedLeft }),
            width: Math.round(r.width),
            // The popup can use max-content, so its final width is not known yet.
            // Keep enough room for the opposite viewport padding once anchored.
            maxPopupWidth: Math.max(
                0,
                Math.floor(window.innerWidth - PAD - (alignRight ? clampedRight : clampedLeft))
            ),
            maxListH,
        };
    ...
    const popStyle = useMemo(() => {
        ...
        const resolvedMinWidth = menuMinWidth != null
            ...
            : null;
        const maxPopupWidth = Math.max(0, pos.maxPopupWidth ?? (window.innerWidth - 16));
        const requestedMinWidth = resolvedMinWidth != null
            ? Number.parseFloat(resolvedMinWidth)
            : pos.width;
        // A min-width larger than max-width wins in CSS and would reintroduce
        // horizontal overflow, so cap both values to the available width.
        const clampedMinWidth = `${Math.min(requestedMinWidth || 0, maxPopupWidth)}px`;

        const base = {
            ...(pos.right != null
                ? { left: "auto", right: pos.right }
                : { left: pos.left, right: "auto" }),
            width: pos.width,
            maxWidth: `${maxPopupWidth}px`,
            ...(pos.top != null ? { top: pos.top, bottom: "auto" } : { top: "auto", bottom: pos.bottom }),
            "--cd-list-maxh": `${pos.maxListH ?? 450}px`,
        };

        if (width == null) {
            return { ...base, minWidth: clampedMinWidth, width: "max-content" };
        }

        if (resolvedMinWidth) {
            return { ...base, minWidth: clampedMinWidth };
        }
        return base;
    ...
    return (
        <div ref={rootRef} className={rootClassName} style={rootStyle} data-open={open ? "1" : "0"} data-help-id={helpId || undefined}>
            {title && (
