...
function getLocaleWithFallback(language) {
  ...
  return {
    ...
    baseSteps: Array.isArray(selected.baseSteps) && selected.baseSteps.length
      ? selected.baseSteps
      : (fallback.baseSteps || []),
    featureSteps: Array.isArray(selected.featureSteps) && selected.featureSteps.length
      ? selected.featureSteps
      : (fallback.featureSteps || []),
  };
}
...
function buildFeatureSteps(locale) {
  return Array.isArray(locale.featureSteps) ? locale.featureSteps : [];
}
...
function PageCoach({ onClose, currentPage, initialMode = "page" }) {
  ...
  const steps = useMemo(() => {
    if (mode === "page") return buildPageSteps(currentPage, locale);
    if (mode === "features") return buildFeatureSteps(locale);
    return buildBaseSteps(locale);
  }, [currentPage, mode, domTick, locale]);
  ...
  const bubbleWidth = Math.min(mode === "features" ? 640 : 310, window.innerWidth - 20);
  const activeRect = mode === "features" ? null : rect;
  const bubblePos = activeRect ? computeBubblePos(activeRect, bubbleWidth, bubbleHeight) : { left: 10, top: Math.max(10, window.innerHeight - bubbleHeight - 10), width: bubbleWidth };

  return (
    <div className="pagecoach-overlay" onClick={onClose}>
      {activeRect ? <div className="pagecoach-focus" style={{ left: activeRect.left, top: activeRect.top, width: activeRect.width, height: activeRect.height }} /> : null}
      <section ref={bubbleRef} className="pagecoach-bubble" style={{ left: bubblePos.left, top: bubblePos.top, width: bubblePos.width }} onClick={(event) => event.stopPropagation()}>
        <div className="pagecoach-content" key={`${mode}-${language}-${step?.id || index}`}>
          <div className="pagecoach-mode-row">
            ...
            <button type="button" className={`pagecoach-mode-btn ${mode === "features" ? "is-active" : ""}`} onClick={() => setMode("features")}>{locale.ui.featuresMode}</button>
            <div
          ...
          {step?.image ? <img className="pagecoach-feature-image" src={step.image} alt={step.imageAlt || step.title || ""} /> : null}
          {mode !== "features" && !rect ? <p className="pagecoach-warning">{locale.ui.missingZone}</p> : null}
